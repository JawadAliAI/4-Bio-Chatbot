...
PATIENT_DATA_FOLDER = os.getenv(
    "PATIENT_DATA_FOLDER", 
    r"C:\Users\JAY\Downloads\NEWDOC\40biomarker\patient_histories"
)
os.makedirs(PATIENT_DATA_FOLDER, exist_ok=True)

# Biomarker API Configuration
BIOMARKER_API_URL = "http://localhost:8000/predict"
...
def call_biomarker_api(biomarker_data: Dict[str, Any]) -> Dict:
    """Call the biomarker analysis API."""
    try:
        response = requests.post(BIOMARKER_API_URL, json=biomarker_data, timeout=120)
        response.raise_for_status()
        return response.json()
    except Exception as e:
        print(f"❌ Biomarker API error: {str(e)}")
        return None
...
async def chat(request: ChatRequest):
    try:
        ...
        if not biomarker_report and request.biomarker_data:
            ...
            biomarker_report = call_biomarker_api(request.biomarker_data)
...
VOSK_MODEL_PATH_EN = os.getenv(
    "VOSK_MODEL_EN",
    r"C:\Users\JAY\Downloads\Chatdoc\newfrontend\vosk-model-small-en-us-0.15"
)
VOSK_MODEL_PATH_AR = os.getenv(
    "VOSK_MODEL_AR",
    r"C:\Users\JAY\Downloads\Chatdoc\newfrontend\vosk-model-ar-mgb2-0.4"
)
...
async def text_to_speech(req: TTSRequest):
    try:
        ...
        tts = gTTS(text=req.text.strip(), lang=detected_lang, slow=False)
...
def root():
    return {
        "message": "Dr. HealBot Stateless API",
        "version": "4.0",
        "features": [
